...
export const SingaporeAnalysis: React.FC = () => {
  ...
  const sgCountry = getCountryByCode('sg');
  const sgData = sgCountry ? getCountryData(sgCountry.hqCode) : [];

  // Calculate key metrics
  const avgHealthApps = sgData.reduce((sum, d) => sum + (d.B4 || 0), 0) / sgData.length;
  const avgWearables = sgData.reduce((sum, d) => sum + (d.B5 || 0), 0) / sgData.length;
  const avgMentalHealth = sgData.reduce((sum, d) => {
  ...
  const excellentHealthPercent = Math.round((sgData.filter(d => d.B3 === 5 || d.B3 === 4).length / sgData.length) * 100);

  // Demographics data
  const ethnicityLabels = { '1': 'Chinese', '2': 'Malay', '3': 'Indian', '4': 'Eurasian', '5': 'Others' };
  const genderLabels = { '1': 'Male', '2': 'Female', '3': 'Others' };
  
  const ethnicityData = Object.entries(ethnicityLabels).map(([code, label]) => ({
    name: label,
    value: sgData.filter(d => d.A4 === parseInt(code)).length,
    percentage: parseFloat(((sgData.filter(d => d.A4 === parseInt(code)).length / sgData.length) * 100).toFixed(2))
  }));
  ...
  const genderData = Object.entries(genderLabels).map(([code, label]) => ({
    name: label,
    value: sgData.filter(d => d.A3 === parseInt(code)).length,
    percentage: parseFloat(((sgData.filter(d => d.A3 === parseInt(code)).length / sgData.length) * 100).toFixed(2))
  }));

  // High usage segments
  const highTechUsers = sgData.filter(d => d.B4 > 3 || d.B5 > 3).length;
  const highTechPercent = Math.round((highTechUsers / sgData.length) * 100);

  // Income distribution - A5 field
  // const highIncomeUsers = sgData.filter(d => d.A5 > 15).length;
  // const highIncomePercent = Math.round((highIncomeUsers / sgData.length) * 100);
  const nutritionAwarenessPercent = Math.round((sgData.filter(d => d.B6 >= 4).length / sgData.length) * 100);
  ...
  const ageStats = calculateAgeStats(sgData);
  ...
  const physicalHealthData = [
    {
      name: 'Excellent',
      value: sgData.filter(d => d.B3 === 5).length,
      percentage: Math.round((sgData.filter(d => d.B3 === 5).length / sgData.length) * 100),
      fill: '#10b981'
    },
    {
      name: 'Very Good',
      value: sgData.filter(d => d.B3 === 4).length,
      percentage: Math.round((sgData.filter(d => d.B3 === 4).length / sgData.length) * 100),
      fill: '#3b82f6'
    },
    {
      name: 'Good',
      value: sgData.filter(d => d.B3 === 3).length,
      percentage: Math.round((sgData.filter(d => d.B3 === 3).length / sgData.length) * 100),
      fill: '#f59e0b'
    },
    {
      name: 'Fair',
      value: sgData.filter(d => d.B3 === 2).length,
      percentage: Math.round((sgData.filter(d => d.B3 === 2).length / sgData.length) * 100),
      fill: '#f97316'
    },
    {
      name: 'Poor',
      value: sgData.filter(d => d.B3 === 1).length,
      percentage: Math.round((sgData.filter(d => d.B3 === 1).length / sgData.length) * 100),
      fill: '#ef4444'
    }
  ...
  const calculateEducationStats = (data: HealthData[]) => {
    ...
    // Tertiary education = A6 >= 6 (A-level/Diploma and above)
  ...
  const educationStats = calculateEducationStats(sgData);

  // Calculate median income (A5 field - convert to approximate income values)
  const calculateMedianIncome = (data: HealthData[]) => {
    if (data.length === 0) return 0;
    
    // Map income codes to midpoint values of each range
    const incomeMap: { [key: number]: number } = {
      1: 500,    // Less than S$1,000 (midpoint: S$500)
      2: 1500,   // S$1,000 to S$1,999 (midpoint: S$1,500)
      3: 2500,   // S$2,000 to S$2,999 (midpoint: S$2,500)
      4: 3500,   // S$3,000 to S$3,999 (midpoint: S$3,500)
      5: 4500,   // S$4,000 to S$4,999 (midpoint: S$4,500)
      6: 5500,   // S$5,000 to S$5,999 (midpoint: S$5,500)
      7: 6500,   // S$6,000 to S$6,999 (midpoint: S$6,500)
      8: 7500,   // S$7,000 to S$7,999 (midpoint: S$7,500)
      9: 8500,   // S$8,000 to S$8,999 (midpoint: S$8,500)
      10: 9500,  // S$9,000 to S$9,999 (midpoint: S$9,500)
      11: 10500, // S$10,000 to S$10,999 (midpoint: S$10,500)
      12: 11500, // S$11,000 to S$11,999 (midpoint: S$11,500)
      13: 12500, // S$12,000 to S$12,999 (midpoint: S$12,500)
      14: 13500, // S$13,000 to S$13,999 (midpoint: S$13,500)
      15: 14500, // S$14,000 to S$14,999 (midpoint: S$14,500)
      16: 15500, // S$15,000 to S$15,999 (midpoint: S$15,500)
      17: 16500, // S$16,000 to S$16,999 (midpoint: S$16,500)
      18: 17500, // S$17,000 to S$17,999 (midpoint: S$17,500)
      19: 18500, // S$18,000 to S$18,999 (midpoint: S$18,500)
      20: 19500, // S$19,000 to S$19,999 (midpoint: S$19,500)
      21: 22000  // S$20,000 and above (estimated midpoint: S$22,000)
    };
  ...
  const medianIncome = calculateMedianIncome(sgData);

  // Health Metrics by Income with actual income ranges
  const calculateHealthMetricsByIncome = (data: HealthData[]) => {
    // Define income ranges with actual labels
    const incomeRanges = [
      { min: 1, max: 3, label: 'S$<3K', description: 'Under S$3,000' },
      { min: 4, max: 6, label: 'S$3K-6K', description: 'S$3,000 - S$6,999' },
      { min: 7, max: 9, label: 'S$7K-9K', description: 'S$7,000 - S$9,999' },
      { min: 10, max: 13, label: 'S$10K-13K', description: 'S$10,000 - S$13,999' },
      { min: 14, max: 16, label: 'S$14K-16K', description: 'S$14,000 - S$16,999' },
      { min: 17, max: 21, label: 'S$17K+', description: 'S$17,000 and above' }
    ];
  ...
  const healthMetricsByIncome = calculateHealthMetricsByIncome(sgData);
  ...
  const techImpactProxies = calculateTechImpactProxies(sgData);

  // Calculate key findings metrics
  const techAdoptionRate = calculateTechUsageStats(sgData).dailyAppUsers;
  const highIncomePercent = Math.round((sgData.filter(d => d.A5 >= 17).length / sgData.length) * 100);
  const wearableAdoption = calculateTechUsageStats(sgData).wearableOwners;
  const incomeHealthGap = healthMetricsByIncome.length > 0 
    ...
    : 0;
  const mentalWellbeingPercent = ((sgData.reduce((sum, d) => {
    const emotional = (d.B11_1 + d.B11_2 + d.B11_3) / 3;
    const psychological = (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6;
    const social = (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5;
    return sum + (emotional + psychological + social) / 3;
  }, 0) / sgData.length) / 6 * 100).toFixed(1);
  ...
  // Calculate the stats
  const appsWearablesStats = calculateAppsWearablesStats(sgData);
  const socialEnvironmentStats = calculateSocialEnvironmentStats(sgData);
    ...
    <div className="space-y-8 bg-gradient-to-br from-blue-50 to-indigo-50 p-8 rounded-2xl">
      ...
      <div className="text-center bg-gray-200 p-8 rounded-2xl shadow-sm">
        <div className="flex items-center justify-center gap-3 mb-4">
          <FlagImage code='sg' size='xxl'/>
          <h1 className="text-4xl font-bold text-gray-900">Singapore</h1>
        </div>
        <h2 className="text-2xl font-semibold text-gray-700 mb-2">Digital Health Technology Analysis</h2>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Understanding how technology is transforming healthcare experiences across Singapore's diverse population
        </p>
      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-4xl font-bold mb-2">{sgData.length}</div>
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <div className="text-white text-xs opacity-80 mt-2">
                        Based on {sgData.length} participants
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white p-8 rounded-2xl">
                <h3 className="text-2xl font-bold mb-6 text-center">Singapore's Digital Health Journey</h3>
            ...
            <div className="space-y-8">
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-white text-sm space-y-2">
                          {(() => {
                            const ageGroups = {
                              '18-24': sgData.filter(d => d.A1 >= 18 && d.A1 <= 24).length,
                              '25-34': sgData.filter(d => d.A1 >= 25 && d.A1 <= 34).length,
                              '35-44': sgData.filter(d => d.A1 >= 35 && d.A1 <= 44).length,
                              '45-54': sgData.filter(d => d.A1 >= 45 && d.A1 <= 54).length,
                              '55+': sgData.filter(d => d.A1 >= 55).length
                            };
                            ...
                            return Object.entries(ageGroups).map(([group, count]) => (
                              <div key={group} className="flex justify-between">
                                ...
                                <span className="font-bold ml-4">
                                  {count} ({(count / sgData.length * 100).toFixed(1)}%)
                                </span>
                              </div>
                            ));
                          })()}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                transition-all duration-300 hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-3xl font-bold text-green-600 mb-2">S${medianIncome.toLocaleString()}</div>
                    ...
                    <div className="w-full space-y-1.5 max-h-56 overflow-y-auto">
                      ...
                      {(() => {
                        const incomeBrackets = {
                          'Low (<S$3K)': sgData.filter(d => d.A5 <= 3).length,
                          'Middle (S$3K-8K)': sgData.filter(d => d.A5 >= 4 && d.A5 <= 9).length,
                          'Upper Middle (S$8K-15K)': sgData.filter(d => d.A5 >= 10 && d.A5 <= 16).length,
                          'High (>S$15K)': sgData.filter(d => d.A5 >= 17).length
                        };
                            ...
                            <div className="flex-1 mx-1.5">
                              <div className="bg-gray-600 rounded-full h-1">
                                <div 
                                  ...
                                  style={{ width: `${(count / sgData.length) * 100}%` }}
                                ></div>
                              </div>
                            </div>
                            <span className="font-bold w-10 text-right">
                              {((count / sgData.length) * 100).toFixed(1)}%
                            </span>
                          </div>
                        ));
                      })()}
                      ...
                      <div className="border-t border-gray-600 pt-1 mt-1 text-[10px] text-gray-300">
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold">
                            {(() => {
                              const highIncomeCount = sgData.filter(d => d.A5 >= 17).length;
                              return `${((highIncomeCount / sgData.length) * 100).toFixed(1)}%`;
                            })()}
                          </span>
                        </div>
                        <div className="flex justify-between mt-0.5">
                          <span>Income Diversity:</span>
                          <span className="font-bold">
                            {(() => {
                              const uniqueIncomeLevels = new Set(sgData.map(d => d.A5)).size;
                              return `${uniqueIncomeLevels} levels`;
                            })()}
                          </span>
                        </div>
                      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{calculateTechUsageStats(sgData).dailyAppUsers}%</div>
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <div className="text-white text-xs opacity-80 mt-2">
                        Survey sample: {sgData.length} participants
                      </div>
                    </div>
                  </div>
                </div>
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{calculateTechUsageStats(sgData).wearableOwners}%</div>
                  <div className="text-purple-100 font-medium mb-2">Own Wearable Devices</div>
                  <div className="text-sm text-purple-200">Growing {Math.round(calculateTechUsageStats(sgData).wearableOwners / 3)}% annually</div>
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{formatHoursAndMinutes(calculateTechUsageStats(sgData).avgDailyUsage)}</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>Regular Activity: {Math.round((sgData.filter(d => d.B4 >= 3).length / sgData.length) * 100)}%</div>
                      <div>Nutrition Aware: {Math.round((sgData.filter(d => d.B6 >= 4).length / sgData.length) * 100)}%</div>
                      <div>Health Conscious: {Math.round((sgData.filter(d => d.B3 >= 4).length / sgData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>High Emotional: {Math.round((sgData.filter(d => (d.B11_1 + d.B11_2 + d.B11_3) / 3 >= 4).length / sgData.length) * 100)}%</div>
                      <div>High Psychological: {Math.round((sgData.filter(d => (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6 >= 4).length / sgData.length) * 100)}%</div>
                      <div>High Social: {Math.round((sgData.filter(d => (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5 >= 4).length / sgData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>Excellent Health: {Math.round((sgData.filter(d => d.B3 === 5).length / sgData.length) * 100)}%</div>
                      <div>Good Health: {Math.round((sgData.filter(d => d.B3 === 4).length / sgData.length) * 100)}%</div>
                      <div>Fair Health: {Math.round((sgData.filter(d => d.B3 === 3).length / sgData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>No Conditions: {Math.round((sgData.filter(d => d.B10_9 === 1).length / sgData.length) * 100)}%</div>
                      <div>Non-smokers: {Math.round((sgData.filter(d => d.B8 === 1).length / sgData.length) * 100)}%</div>
                      <div>Low Alcohol: {Math.round((sgData.filter(d => d.B7 <= 2).length / sgData.length) * 100)}%</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-gradient-to-br from-blue-500 to-cyan-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-3xl font-bold mb-1">{((sgData.reduce((sum, d) => {
                    const emotional = (d.B11_1 + d.B11_2 + d.B11_3) / 3;
                    const psychological = (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6;
                    const social = (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5;
                    return sum + (emotional + psychological + social) / 3;
                  }, 0) / sgData.length) / 6 * 100).toFixed(1)}%</div>
                ...
                <div className="bg-gradient-to-br from-purple-500 to-violet-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-3xl font-bold mb-1">
                    {Math.round((sgData.filter(d => d.B4 >= 3).length / sgData.length) * 100)}%
                  </div>
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-blue-600">
                          {convertToPercentage(
                            sgData.reduce((sum, d) => sum + (d.B11_1 + d.B11_2 + d.B11_3) / 3, 0) / sgData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-blue-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(sgData.reduce((sum, d) => sum + (d.B11_1 + d.B11_2 + d.B11_3) / 3, 0) / sgData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-green-600">
                          {convertToPercentage(
                            sgData.reduce((sum, d) => sum + (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6, 0) / sgData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-green-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(sgData.reduce((sum, d) => sum + (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6, 0) / sgData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
                    <div className="bg-purple-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-purple-600">
                          {convertToPercentage(
                            sgData.reduce((sum, d) => sum + (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5, 0) / sgData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-purple-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(sgData.reduce((sum, d) => sum + (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5, 0) / sgData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
      ...
      <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-pink-900 text-white p-8 rounded-2xl">
        <h3 className="text-3xl font-bold mb-6 text-center">Singapore: Leading the Digital Health Revolution</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
