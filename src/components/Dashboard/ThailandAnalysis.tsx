...
export const ThailandAnalysis: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'demographics' | 'technology' | 'health' | 'apps_wearables' | 'social_environment'>('overview');
  
  // Filter Thailand data
  const thCountry = getCountryByCode('th');
  const thData = thCountry ? getCountryData(thCountry.hqCode) : [];

  // Calculate key metrics
  const avgHealthApps = thData.reduce((sum, d) => sum + (d.B4 || 0), 0) / thData.length;
  const avgWearables = thData.reduce((sum, d) => sum + (d.B5 || 0), 0) / thData.length;
  const avgMentalHealth = thData.reduce((sum, d) => {
  ...
  const excellentHealthPercent = Math.round((thData.filter(d => d.B3 === 5 || d.B3 === 4).length / thData.length) * 100);
  ...
  const ethnicityData = Object.entries(ethnicityLabels).map(([code, label]) => ({
    name: label,
    value: thData.filter(d => d.A4 === parseInt(code)).length,
    percentage: parseFloat(((thData.filter(d => d.A4 === parseInt(code)).length / thData.length) * 100).toFixed(2))
  }));
  ...
  const genderData = Object.entries(genderLabels).map(([code, label]) => ({
    name: label,
    value: thData.filter(d => d.A3 === parseInt(code)).length,
    percentage: parseFloat(((thData.filter(d => d.A3 === parseInt(code)).length / thData.length) * 100).toFixed(2))
  }));

  // High usage segments
  const highTechUsers = thData.filter(d => d.B4 > 3 || d.B5 > 3).length;
  const highTechPercent = Math.round((highTechUsers / thData.length) * 100);

  // Income distribution - A5 field
  // const highIncomeUsers = thData.filter(d => d.A5 > 15).length;
  // const highIncomePercent = Math.round((highIncomeUsers / thData.length) * 100);
  const nutritionAwarenessPercent = Math.round((thData.filter(d => d.B6 >= 4).length / thData.length) * 100);
  ...
  const ageStats = calculateAgeStats(thData);

  // Education labels based on the survey
  const educationLabels: { [key: number]: string } = {
    1: 'ไม่ได้เข้ารับการศึกษา',
    2: 'ประถม 6 หรือต่ำกว่า',
    3: 'มัธยม 1 ถึง 3',
    4: 'มัธยม 4 ถึง 6',
    5: 'ปริญญาตรี',
    6: 'ปริญญาโท',
  ...
  const physicalHealthData = [
    {
      name: 'Excellent',
      value: thData.filter(d => d.B3 === 5).length,
      percentage: Math.round((thData.filter(d => d.B3 === 5).length / thData.length) * 100),
      fill: '#10b981'
    },
    {
      name: 'Very Good',
      value: thData.filter(d => d.B3 === 4).length,
      percentage: Math.round((thData.filter(d => d.B3 === 4).length / thData.length) * 100),
      fill: '#3b82f6'
    },
    {
      name: 'Good',
      value: thData.filter(d => d.B3 === 3).length,
      percentage: Math.round((thData.filter(d => d.B3 === 3).length / thData.length) * 100),
      fill: '#f59e0b'
    },
    {
      name: 'Fair',
      value: thData.filter(d => d.B3 === 2).length,
      percentage: Math.round((thData.filter(d => d.B3 === 2).length / thData.length) * 100),
      fill: '#f97316'
    },
    {
      name: 'Poor',
      value: thData.filter(d => d.B3 === 1).length,
      percentage: Math.round((thData.filter(d => d.B3 === 1).length / thData.length) * 100),
      fill: '#ef4444'
    }
  ].filter(item => item.value > 0); // Only show categories that have data

  // Calculate education statistics for Thailand
  const calculateEducationStats = (data: HealthData[]) => {
    if (data.length === 0) return { tertiaryPercent: 0, distribution: [] };

    // Tertiary education = A6 >= 5 (ปริญญาตรี and above)
  ...
  const educationStats = calculateEducationStats(thData);

  // Calculate median income (A5 field - convert to approximate income values)
  const calculateMedianIncome = (data: HealthData[]) => {
    if (data.length === 0) return 0;
    
    // Map income codes to midpoint values of each range for Thailand (THB)
    const incomeMap: { [key: number]: number } = {
      1: 10000,   // น้อยกว่า ฿20,000 (midpoint: ฿10,000)
      2: 25000,   // ฿20,000 - ฿29,999 (midpoint: ฿25,000)
      3: 35000,   // ฿30,000 - ฿39,999 (midpoint: ฿35,000)
      4: 45000,   // ฿40,000 - ฿49,999 (midpoint: ฿45,000)
      5: 55000,   // ฿50,000 - ฿59,999 (midpoint: ฿55,000)
      6: 65000,   // ฿60,000 - ฿69,999 (midpoint: ฿65,000)
      7: 75000,   // ฿70,000 - ฿79,999 (midpoint: ฿75,000)
      8: 85000,   // ฿80,000 - ฿89,999 (midpoint: ฿85,000)
      9: 95000,   // ฿90,000 - ฿99,999 (midpoint: ฿95,000)
      10: 125000, // ฿100,000 - ฿149,999 (midpoint: ฿125,000)
      11: 175000, // ฿150,000 - ฿199,999 (midpoint: ฿175,000)
      12: 225000, // ฿200,000 - ฿249,999 (midpoint: ฿225,000)
      13: 275000, // ฿250,000 - ฿299,999 (midpoint: ฿275,000)
      14: 325000, // ฿300,000 - ฿349,999 (midpoint: ฿325,000)
      15: 375000, // ฿350,000 - ฿399,999 (midpoint: ฿375,000)
      16: 425000, // ฿400,000 - ฿449,999 (midpoint: ฿425,000)
      17: 475000, // ฿450,000 - ฿499,999 (midpoint: ฿475,000)
      18: 550000  // ฿500,000 ขึ้นไป (estimated midpoint: ฿550,000)
    };
  ...
  const medianIncome = calculateMedianIncome(thData);

  // Health Metrics by Income with actual income ranges for Thailand
  const calculateHealthMetricsByIncome = (data: HealthData[]) => {
    // Define income ranges with actual labels for Thailand (THB)
    const incomeRanges = [
      { min: 1, max: 3, label: 'THB<40K', description: 'Under THB 40,000' },
      { min: 4, max: 6, label: 'THB40K-70K', description: 'THB 40,000 - THB 69,999' },
      { min: 7, max: 9, label: 'THB70K-100K', description: 'THB 70,000 - THB 99,999' },
      { min: 10, max: 11, label: 'THB100K-200K', description: 'THB 100,000 - THB 199,999' },
      { min: 12, max: 14, label: 'THB200K-350K', description: 'THB 200,000 - THB 349,999' },
      { min: 15, max: 18, label: 'THB350K+', description: 'THB 350,000 and above' }
    ];
  ...
  const healthMetricsByIncome = calculateHealthMetricsByIncome(thData);
  ...
  const techImpactProxies = calculateTechImpactProxies(thData);

  // Calculate key findings metrics
  const techAdoptionRate = calculateTechUsageStats(thData).dailyAppUsers;
  const highIncomePercent = Math.round((thData.filter(d => d.A5 >= 17).length / thData.length) * 100);
  const wearableAdoption = calculateTechUsageStats(thData).wearableOwners;
  const incomeHealthGap = healthMetricsByIncome.length > 0 
    ...
    : 0;
  const mentalWellbeingPercent = ((thData.reduce((sum, d) => {
    const emotional = (d.B11_1 + d.B11_2 + d.B11_3) / 3;
    const psychological = (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6;
    const social = (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5;
    return sum + (emotional + psychological + social) / 3;
  }, 0) / thData.length) / 6 * 100).toFixed(1);
  ...
  // Calculate the stats
  const appsWearablesStats = calculateAppsWearablesStats(thData);
  const socialEnvironmentStats = calculateSocialEnvironmentStats(thData);
    ...
    <div className="space-y-8 bg-gradient-to-br from-blue-50 to-indigo-50 p-8 rounded-2xl">
      ...
      <div className="text-center bg-gray-200 p-8 rounded-2xl shadow-sm">
        <div className="flex items-center justify-center gap-3 mb-4">
          <FlagImage code='th' size='xxl'/>
          <h1 className="text-4xl font-bold text-gray-900">Thailand</h1>
        </div>
        <h2 className="text-2xl font-semibold text-gray-700 mb-2">Digital Health Technology Analysis</h2>
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Understanding how technology is transforming healthcare experiences across Thailand's diverse population
        </p>
      </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-4xl font-bold mb-2">{thData.length}</div>
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <div className="text-white text-xs opacity-80 mt-2">
                        Based on {thData.length} participants
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white p-8 rounded-2xl">
                <h3 className="text-2xl font-bold mb-6 text-center">Thailand's Digital Health Journey</h3>
            ...
            <div className="space-y-8">
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-white text-sm space-y-2">
                          {(() => {
                            const ageGroups = {
                              '18-24': thData.filter(d => d.A1 >= 18 && d.A1 <= 24).length,
                              '25-34': thData.filter(d => d.A1 >= 25 && d.A1 <= 34).length,
                              '35-44': thData.filter(d => d.A1 >= 35 && d.A1 <= 44).length,
                              '45-54': thData.filter(d => d.A1 >= 45 && d.A1 <= 54).length,
                              '55+': thData.filter(d => d.A1 >= 55).length
                            };
                            ...
                            return Object.entries(ageGroups).map(([group, count]) => (
                              <div key={group} className="flex justify-between">
                                ...
                                <span className="font-bold ml-4">
                                  {count} ({(count / thData.length * 100).toFixed(1)}%)
                                </span>
                              </div>
                            ));
                          })()}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                transition-all duration-300 hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-3xl font-bold text-green-600 mb-2">THB {medianIncome.toLocaleString()}</div>
                    ...
                    <div className="w-full space-y-1.5 max-h-56 overflow-y-auto">
                      ...
                      {(() => {
                        const incomeBrackets = {
                          'Low (<THB40K)': thData.filter(d => d.A5 <= 3).length,
                          'Middle (THB40K-70K)': thData.filter(d => d.A5 >= 4 && d.A5 <= 6).length,
                          'Upper Middle (THB70K-100K)': thData.filter(d => d.A5 >= 7 && d.A5 <= 9).length,
                          'High (>THB100K)': thData.filter(d => d.A5 >= 10).length
                        };
                            ...
                            <div className="flex-1 mx-1.5">
                              <div className="bg-gray-600 rounded-full h-1">
                                <div 
                                  ...
                                  style={{ width: `${(count / thData.length) * 100}%` }}
                                ></div>
                              </div>
                            </div>
                            <span className="font-bold w-10 text-right">
                              {((count / thData.length) * 100).toFixed(1)}%
                            </span>
                          </div>
                        ));
                      })()}
                      ...
                      <div className="border-t border-gray-600 pt-1 mt-1 text-[10px] text-gray-300">
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold">
                            {(() => {
                              const maxIncomeCode = Math.max(...thData.map(d => d.A5));
                              const maxGroups = thData.filter(d => d.A5 === maxIncomeCode).length;
                        ...
                        <div className="flex justify-between mt-0.5">
                          <span>Range:</span>
                          <span className="font-bold">
                            {Math.min(...thData.map(d => d.A5))}-{Math.max(...thData.map(d => d.A5))}/18
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{calculateTechUsageStats(thData).dailyAppUsers}%</div>
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <div className="text-white text-xs opacity-80 mt-2">
                        Survey sample: {thData.length} participants
                      </div>
                    </div>
                  </div>
                </div>
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{calculateTechUsageStats(thData).wearableOwners}%</div>
                  <div className="text-purple-100 font-medium mb-2">Own Wearable Devices</div>
                  <div className="text-sm text-purple-200">Growing {Math.round(calculateTechUsageStats(thData).wearableOwners / 3)}% annually</div>
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{formatHoursAndMinutes(calculateTechUsageStats(thData).avgDailyUsage)}</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>Regular Activity: {Math.round((thData.filter(d => d.B4 >= 3).length / thData.length) * 100)}%</div>
                      <div>Nutrition Aware: {Math.round((thData.filter(d => d.B6 >= 4).length / thData.length) * 100)}%</div>
                      <div>Health Conscious: {Math.round((thData.filter(d => d.B3 >= 4).length / thData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>High Emotional: {Math.round((thData.filter(d => (d.B11_1 + d.B11_2 + d.B11_3) / 3 >= 4).length / thData.length) * 100)}%</div>
                      <div>High Psychological: {Math.round((thData.filter(d => (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6 >= 4).length / thData.length) * 100)}%</div>
                      <div>High Social: {Math.round((thData.filter(d => (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5 >= 4).length / thData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>Excellent Health: {Math.round((thData.filter(d => d.B3 === 5).length / thData.length) * 100)}%</div>
                      <div>Good Health: {Math.round((thData.filter(d => d.B3 === 4).length / thData.length) * 100)}%</div>
                      <div>Fair Health: {Math.round((thData.filter(d => d.B3 === 3).length / thData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>No Conditions: {Math.round((thData.filter(d => d.B10_9 === 1).length / thData.length) * 100)}%</div>
                      <div>Non-smokers: {Math.round((thData.filter(d => d.B8 === 1).length / thData.length) * 100)}%</div>
                      <div>Low Alcohol: {Math.round((thData.filter(d => d.B7 <= 2).length / thData.length) * 100)}%</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-gradient-to-br from-blue-500 to-cyan-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-3xl font-bold mb-1">{((thData.reduce((sum, d) => {
                    const emotional = (d.B11_1 + d.B11_2 + d.B11_3) / 3;
                    const psychological = (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6;
                    const social = (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5;
                    return sum + (emotional + psychological + social) / 3;
                  }, 0) / thData.length) / 6 * 100).toFixed(1)}%</div>
                ...
                <div className="bg-gradient-to-br from-purple-500 to-violet-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-3xl font-bold mb-1">
                    {Math.round((thData.filter(d => d.B4 >= 3).length / thData.length) * 100)}%
                  </div>
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-blue-600">
                          {convertToPercentage(
                            thData.reduce((sum, d) => sum + (d.B11_1 + d.B11_2 + d.B11_3) / 3, 0) / thData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-blue-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(thData.reduce((sum, d) => sum + (d.B11_1 + d.B11_2 + d.B11_3) / 3, 0) / thData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-green-600">
                          {convertToPercentage(
                            thData.reduce((sum, d) => sum + (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6, 0) / thData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-green-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(thData.reduce((sum, d) => sum + (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6, 0) / thData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
                    <div className="bg-purple-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-purple-600">
                          {convertToPercentage(
                            thData.reduce((sum, d) => sum + (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5, 0) / thData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-purple-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(thData.reduce((sum, d) => sum + (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5, 0) / thData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
      ...
      <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-pink-900 text-white p-8 rounded-2xl">
        <h3 className="text-3xl font-bold mb-6 text-center">Thailand: Leading the Digital Health Revolution</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
