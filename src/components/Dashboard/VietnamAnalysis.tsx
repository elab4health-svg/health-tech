...
export const VietnamAnalysis: React.FC = () => {
  ...
  const vnData = vnCountry ? getCountryData(vnCountry.hqCode) : [];

  // Calculate key metrics
  const avgHealthApps = vnData.reduce((sum, d) => sum + (d.B4 || 0), 0) / vnData.length;
  const avgWearables = vnData.reduce((sum, d) => sum + (d.B5 || 0), 0) / vnData.length;
  const avgMentalHealth = vnData.reduce((sum, d) => {
  ...
  const excellentHealthPercent = Math.round((vnData.filter(d => d.B3 === 5 || d.B3 === 4).length / vnData.length) * 100);
  ...
  const ethnicityData = Object.entries(ethnicityLabels).map(([code, label]) => ({
    name: label,
    value: vnData.filter(d => d.A4 === parseInt(code)).length,
    percentage: parseFloat(((vnData.filter(d => d.A4 === parseInt(code)).length / vnData.length) * 100).toFixed(2))
  }));
  ...
  const genderData = Object.entries(genderLabels).map(([code, label]) => ({
    name: label,
    value: vnData.filter(d => d.A3 === parseInt(code)).length,
    percentage: parseFloat(((vnData.filter(d => d.A3 === parseInt(code)).length / vnData.length) * 100).toFixed(2))
  }));

  // High usage segments
  const highTechUsers = vnData.filter(d => d.B4 > 3 || d.B5 > 3).length;
  const highTechPercent = Math.round((highTechUsers / vnData.length) * 100);

  // Income distribution - A5 field
  // const highIncomeUsers = vnData.filter(d => d.A5 > 15).length;
  // const highIncomePercent = Math.round((highIncomeUsers / vnData.length) * 100);
  const nutritionAwarenessPercent = Math.round((vnData.filter(d => d.B6 >= 4).length / vnData.length) * 100);
  ...
  const ageStats = calculateAgeStats(vnData);

  // Education labels based on the survey
  const educationLabels: { [key: number]: string } = {
    1: 'Không học hành chính thức',
    2: 'Cấp 1 (Tiểu học) hoặc thấp hơn',
    3: 'Cấp 2 (Trung học cơ sở)',
    4: 'Cấp 3 (Trung học phổ thông)',
    5: 'Cử nhân (Đại học/ Cao đẳng)',
    6: 'Thạc sĩ',
    7: 'Tiến sĩ',
    8: 'Khác'
  };
  ...
  const physicalHealthData = [
    {
      name: 'Excellent',
      value: vnData.filter(d => d.B3 === 5).length,
      percentage: Math.round((vnData.filter(d => d.B3 === 5).length / vnData.length) * 100),
      fill: '#10b981'
    },
    {
      name: 'Very Good',
      value: vnData.filter(d => d.B3 === 4).length,
      percentage: Math.round((vnData.filter(d => d.B3 === 4).length / vnData.length) * 100),
      fill: '#3b82f6'
    },
    {
      name: 'Good',
      value: vnData.filter(d => d.B3 === 3).length,
      percentage: Math.round((vnData.filter(d => d.B3 === 3).length / vnData.length) * 100),
      fill: '#f59e0b'
    },
    {
      name: 'Fair',
      value: vnData.filter(d => d.B3 === 2).length,
      percentage: Math.round((vnData.filter(d => d.B3 === 2).length / vnData.length) * 100),
      fill: '#f97316'
    },
    {
      name: 'Poor',
      value: vnData.filter(d => d.B3 === 1).length,
      percentage: Math.round((vnData.filter(d => d.B3 === 1).length / vnData.length) * 100),
      fill: '#ef4444'
    }
  ].filter(item => item.value > 0); // Only show categories that have data

  // Calculate education statistics for Vietnam
  const calculateEducationStats = (data: HealthData[]) => {
    if (data.length === 0) return { tertiaryPercent: 0, distribution: [] };

    // Tertiary education = A6 >= 5 (Cử nhân and above)
  ...
  const educationStats = calculateEducationStats(vnData);

  // Calculate median income (A5 field - convert to approximate income values)
  const calculateMedianIncome = (data: HealthData[]) => {
    if (data.length === 0) return 0;
    
    // Map income codes to midpoint values of each range for Vietnam (VND - millions)
    const incomeMap: { [key: number]: number } = {
      1: 1,      // Dưới 2 triệu đồng (midpoint: 1 triệu)
      2: 3,      // 2 triệu đến 3,9 triệu đồng (midpoint: 3 triệu)
      3: 5,      // 4 triệu đến 5,9 triệu đồng (midpoint: 5 triệu)
      4: 7,      // 6 triệu đến 7,9 triệu đồng (midpoint: 7 triệu)
      5: 9,      // 8 triệu đến 9,9 triệu đồng (midpoint: 9 triệu)
      6: 11,     // 10 triệu đến 11,9 triệu đồng (midpoint: 11 triệu)
      7: 13,     // 12 triệu đến 13,9 triệu đồng (midpoint: 13 triệu)
      8: 15,     // 14 triệu đến 15,9 triệu đồng (midpoint: 15 triệu)
      9: 17,     // 16 triệu đến 17,9 triệu đồng (midpoint: 17 triệu)
      10: 19,    // 18 triệu đến 19,9 triệu đồng (midpoint: 19 triệu)
      11: 21,    // 20 triệu đến 21,9 triệu đồng (midpoint: 21 triệu)
      12: 23,    // 22 triệu đến 23,9 triệu đồng (midpoint: 23 triệu)
      13: 25,    // 24 triệu đến 25,9 triệu đồng (midpoint: 25 triệu)
      14: 27,    // 26 triệu đến 27,9 triệu đồng (midpoint: 27 triệu)
      15: 29,    // 28 triệu đến 29,9 triệu đồng (midpoint: 29 triệu)
      16: 31,    // 30 triệu đến 31,9 triệu đồng (midpoint: 31 triệu)
      17: 33,    // 32 triệu đến 33,9 triệu đồng (midpoint: 33 triệu)
      18: 35,    // 34 triệu đến 35,9 triệu đồng (midpoint: 35 triệu)
      19: 37,    // 36 triệu đến 37,9 triệu đồng (midpoint: 37 triệu)
      20: 39,    // 38 triệu đến 39,9 triệu đồng (midpoint: 39 triệu)
      21: 45     // 40 triệu đồng trở lên (estimated midpoint: 45 triệu)
    };
    ...
    return Math.round(median); // Return in millions of VND
  };

  const medianIncome = calculateMedianIncome(vnData);

  // Health Metrics by Income with actual income ranges for Vietnam
  const calculateHealthMetricsByIncome = (data: HealthData[]) => {
    // Define income ranges with actual labels for Vietnam (VND - millions)
    const incomeRanges = [
      { min: 1, max: 3, label: 'VND<6M', description: 'Dưới 6 triệu đồng' },
      { min: 4, max: 6, label: 'VND6M-12M', description: '6 - 12 triệu đồng' },
      { min: 7, max: 9, label: 'VND12M-18M', description: '12 - 18 triệu đồng' },
      { min: 10, max: 12, label: 'VND18M-24M', description: '18 - 24 triệu đồng' },
      { min: 13, max: 15, label: 'VND24M-30M', description: '24 - 30 triệu đồng' },
      { min: 16, max: 21, label: 'VND30M+', description: '30 triệu đồng trở lên' }
    ];
  ...
  const healthMetricsByIncome = calculateHealthMetricsByIncome(vnData);
  ...
  const techImpactProxies = calculateTechImpactProxies(vnData);

  // Calculate key findings metrics
  const techAdoptionRate = calculateTechUsageStats(vnData).dailyAppUsers;
  const highIncomePercent = Math.round((vnData.filter(d => d.A5 >= 17).length / vnData.length) * 100);
  const wearableAdoption = calculateTechUsageStats(vnData).wearableOwners;
  const incomeHealthGap = healthMetricsByIncome.length > 0 
    ...
    : 0;
  const mentalWellbeingPercent = ((vnData.reduce((sum, d) => {
    const emotional = (d.B11_1 + d.B11_2 + d.B11_3) / 3;
    const psychological = (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6;
    const social = (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5;
    return sum + (emotional + psychological + social) / 3;
  }, 0) / vnData.length) / 6 * 100).toFixed(1);
    ...
    <div className="space-y-8 bg-gradient-to-br from-blue-50 to-indigo-50 p-8 rounded-2xl">
      ...
      <div className="text-center bg-gray-200 p-8 rounded-2xl shadow-sm">
        ...
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Understanding how technology is transforming healthcare experiences across Vietnam's diverse population
        </p>
      </div>
        ...
        <div className="p-8">
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-4xl font-bold mb-2">{vnData.length}</div>
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <div className="text-white text-xs opacity-80 mt-2">
                        Based on {vnData.length} participants
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-gradient-to-r from-teal-500 to-cyan-500 text-white p-8 rounded-2xl">
                <h3 className="text-2xl font-bold mb-6 text-center">Vietnam's Digital Health Journey</h3>
          ...
          {activeTab === 'demographics' && (
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                      ...
                      <div className="text-center">
                        ...
                        <div className="text-white text-sm space-y-2">
                          {(() => {
                            const ageGroups = {
                              '18-24': vnData.filter(d => d.A1 >= 18 && d.A1 <= 24).length,
                              '25-34': vnData.filter(d => d.A1 >= 25 && d.A1 <= 34).length,
                              '35-44': vnData.filter(d => d.A1 >= 35 && d.A1 <= 44).length,
                              '45-54': vnData.filter(d => d.A1 >= 45 && d.A1 <= 54).length,
                              '55+': vnData.filter(d => d.A1 >= 55).length
                            };
                            ...
                            return Object.entries(ageGroups).map(([group, count]) => (
                              <div key={group} className="flex justify-between">
                                ...
                                <span className="font-bold ml-4">
                                  {count} ({(count / vnData.length * 100).toFixed(1)}%)
                                </span>
                              </div>
                            ));
                          })()}
            ...
            transition-all duration-300 hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-3xl font-bold text-green-600 mb-2">{medianIncome} triệu VND</div>
                    ...
                    <div className="w-full space-y-1.5 max-h-56 overflow-y-auto">
                      ...
                      {(() => {
                        const incomeBrackets = {
                          'Low (<vnR3K)': vnData.filter(d => d.A5 <= 3).length,
                          'Middle (vnR3K-8K)': vnData.filter(d => d.A5 >= 4 && d.A5 <= 9).length,
                          'Upper Middle (vnR8K-15K)': vnData.filter(d => d.A5 >= 10 && d.A5 <= 16).length,
                          'High (>vnR15K)': vnData.filter(d => d.A5 >= 17).length
                        };
                            ...
                            <div className="flex-1 mx-1.5">
                              <div className="bg-gray-600 rounded-full h-1">
                                <div 
                                  ...
                                  style={{ width: `${(count / vnData.length) * 100}%` }}
                                ></div>
                              </div>
                            </div>
                            <span className="font-bold w-10 text-right">
                              {((count / vnData.length) * 100).toFixed(1)}%
                            </span>
                          </div>
                        ));
                      })()}
                      ...
                      <div className="border-t border-gray-600 pt-1 mt-1 text-[10px] text-gray-300">
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold">
                            {(() => {
                              const maxIncomeCode = Math.max(...vnData.map(d => d.A5));
                              const maxGroups = vnData.filter(d => d.A5 === maxIncomeCode).length;
                        ...
                        <div className="flex justify-between mt-0.5">
                          <span>Range:</span>
                          <span className="font-bold">
                            {Math.min(...vnData.map(d => d.A5))}-{Math.max(...vnData.map(d => d.A5))}/21
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{calculateTechUsageStats(vnData).dailyAppUsers}%</div>
                  ...
                  <div className="absolute inset-0 bg-black bg-opacity-85 rounded-2xl flex items-center justify-center 
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <div className="text-white text-xs opacity-80 mt-2">
                        Survey sample: {vnData.length} participants
                      </div>
                    </div>
                  </div>
                </div>
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{calculateTechUsageStats(vnData).wearableOwners}%</div>
                  <div className="text-purple-100 font-medium mb-2">Own Wearable Devices</div>
                  <div className="text-sm text-purple-200">Growing {Math.round(calculateTechUsageStats(vnData).wearableOwners / 3)}% annually</div>
                ...
                hover:shadow-lg cursor-pointer group relative">
                  ...
                  <div className="text-4xl font-bold mb-2">{formatHoursAndMinutes(calculateTechUsageStats(vnData).avgDailyUsage)}</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>Regular Activity: {Math.round((vnData.filter(d => d.B4 >= 3).length / vnData.length) * 100)}%</div>
                      <div>Nutrition Aware: {Math.round((vnData.filter(d => d.B6 >= 4).length / vnData.length) * 100)}%</div>
                      <div>Health Conscious: {Math.round((vnData.filter(d => d.B3 >= 4).length / vnData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>High Emotional: {Math.round((vnData.filter(d => (d.B11_1 + d.B11_2 + d.B11_3) / 3 >= 4).length / vnData.length) * 100)}%</div>
                      <div>High Psychological: {Math.round((vnData.filter(d => (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6 >= 4).length / vnData.length) * 100)}%</div>
                      <div>High Social: {Math.round((vnData.filter(d => (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5 >= 4).length / vnData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>Excellent Health: {Math.round((vnData.filter(d => d.B3 === 5).length / vnData.length) * 100)}%</div>
                      <div>Good Health: {Math.round((vnData.filter(d => d.B3 === 4).length / vnData.length) * 100)}%</div>
                      <div>Fair Health: {Math.round((vnData.filter(d => d.B3 === 3).length / vnData.length) * 100)}%</div>
                    </div>
                  </div>
                  <div className="text-center">
                    ...
                    <div className="space-y-1 text-xs">
                      <div>No Conditions: {Math.round((vnData.filter(d => d.B10_9 === 1).length / vnData.length) * 100)}%</div>
                      <div>Non-smokers: {Math.round((vnData.filter(d => d.B8 === 1).length / vnData.length) * 100)}%</div>
                      <div>Low Alcohol: {Math.round((vnData.filter(d => d.B7 <= 2).length / vnData.length) * 100)}%</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-gradient-to-br from-blue-500 to-cyan-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-3xl font-bold mb-1">{((vnData.reduce((sum, d) => {
                    const emotional = (d.B11_1 + d.B11_2 + d.B11_3) / 3;
                    const psychological = (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6;
                    const social = (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5;
                    return sum + (emotional + psychological + social) / 3;
                  }, 0) / vnData.length) / 6 * 100).toFixed(1)}%</div>
                ...
                <div className="bg-gradient-to-br from-purple-500 to-violet-600 text-white p-6 rounded-2xl text-center">
                  ...
                  <div className="text-3xl font-bold mb-1">
                    {Math.round((vnData.filter(d => d.B4 >= 3).length / vnData.length) * 100)}%
                  </div>
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-blue-600">
                          {convertToPercentage(
                            vnData.reduce((sum, d) => sum + (d.B11_1 + d.B11_2 + d.B11_3) / 3, 0) / vnData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-blue-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(vnData.reduce((sum, d) => sum + (d.B11_1 + d.B11_2 + d.B11_3) / 3, 0) / vnData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-green-600">
                          {convertToPercentage(
                            vnData.reduce((sum, d) => sum + (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6, 0) / vnData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-green-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(vnData.reduce((sum, d) => sum + (d.B11_4 + d.B11_9 + d.B11_10 + d.B11_12 + d.B11_13 + d.B11_14) / 6, 0) / vnData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
                    <div className="bg-purple-50 p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className="text-2xl font-bold text-purple-600">
                          {convertToPercentage(
                            vnData.reduce((sum, d) => sum + (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5, 0) / vnData.length,
                            6
                          )}
                        </span>
                      </div>
                      <div className="w-full bg-purple-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ 
                            width: `${(vnData.reduce((sum, d) => sum + (d.B11_5 + d.B11_6 + d.B11_7 + d.B11_8 + d.B11_11) / 5, 0) / vnData.length / 6) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
      ...
      <div className="bg-gradient-to-r from-indigo-900 via-purple-900 to-pink-900 text-white p-8 rounded-2xl">
        <h3 className="text-3xl font-bold mb-6 text-center">Vietnam: Leading the Digital Health Revolution</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
